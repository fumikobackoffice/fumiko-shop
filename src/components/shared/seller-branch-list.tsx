            ...
            <div key={branch.id} className={cn(
              ...
              <div className="space-y-2 flex-1">
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  <Badge variant="outline" className="font-mono text-[10px] whitespace-nowrap shrink-0">{branch.branchCode}</Badge>
                  <Badge variant={getStatusVariant(branch.status)} className="h-6 text-[10px] px-2.5 whitespace-nowrap shrink-0">{getStatusText(branch.status)}</Badge>
                  <Badge variant="secondary" className="h-6 text-[10px] px-2.5 whitespace-nowrap shrink-0">{branch.type === 'MAIN' ? 'สาขาแม่' : 'สาขาลูก'}</Badge>
                  
                  {isExpired ? (
                      <Badge variant="destructive" className="h-6 text-[10px] animate-pulse px-2.5 whitespace-nowrap shrink-0">
                          <AlertTriangle className="mr-1.5 h-3 w-3" /> หมดอายุสัญญา
                      </Badge>
                  ) : isExpiringSoon && (
                      <Badge variant="warning" className="h-6 text-[10px] bg-orange-500 text-white border-none px-2.5 whitespace-nowrap shrink-0">
                          <AlertTriangle className="mr-1.5 h-3 w-3" /> จะหมดอายุใน {daysToExpiry} วัน
                      </Badge>
                  )}
                </div>
